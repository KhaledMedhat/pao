...
import { IconChevronDown, IconChevronRight, IconCopy, IconHeadphonesFilled, IconIdBadge, IconMicrophone, IconMinus, IconMoon, IconPencil, IconPlus, IconSettings } from "@tabler/icons-react";
...
import { STATUS_DURATIONS } from "~/constants/constants";
...
const UserNavigator = () => {
  ...
  const userStatusSwitcher = (status: StatusType) => {
    switch (status) {
      case StatusType.Online:
        return { icon: <span className="bg-[#43a25a] size-4 rounded-full"></span>, label: "Online" };
      case StatusType.Invisible:
        return { icon: <span className="bg-main border-4 border-muted-foreground/40 size-4 rounded-full"></span>, label: "Invisible" };
      case StatusType.DoNotDisturb:
        return {
          icon: <span className="bg-red-500 size-4 rounded-full">
            <IconMinus className="size-full p-0.5 text-background" strokeWidth={4} />
          </span>, label: "Do Not Disturb"
        };
      case StatusType.Idle:
        return { icon: <IconMoon className="size-4 text-yellow-500 fill-yellow-500" />, label: "Idle" };
      default:
        return { icon: <span className="bg-[#43a25a] size-4 rounded-full"></span>, label: "Online" };
    }
  }
  return (
    <Dialog>
      <div className="absolute bottom-4 left-0 right-0 mx-2 flex items-center flex-col gap-2 pr-2 bg-main-primary rounded-md">
        ...
        <div className="flex items-center justify-between">
          <Popover onOpenChange={(open) => {
            ...
            <PopoverTrigger asChild>
              <div className="flex items-center w-fit group gap-2 p-2">
                <div className="flex items-center gap-2 cursor-pointer hover:bg-muted-foreground/15 px-2 py-1 rounded-md">
